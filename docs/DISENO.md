...
| **El blob de arranque** (D18, D19, D20) | Andando en las dos. El firmware trae `blob.bin` de la misma partición de la que salió el kernel —**lo único que sabe leer FAT32 es él** (D25), así que se carga dentro de la ventana y antes de pedir el mapa— y el kernel lo corre con la maquinaria de `exec`: si falla, el fault vuelve como dato y el arranque sigue hasta el protocolo. Antes de saltar avisa por el cable y espera: **cualquier byte lo cancela**, que es lo que hace que un blob roto no deje la máquina inútil en cada arranque. **Y el blob le puede pedir cosas al kernel**, por la **puerta de servicio** (`int 0x81` / `svc #1`): deja el pedido en un buffer suyo, el kernel atiende **uno** y le devuelve el control en la instrucción siguiente. No es un mecanismo nuevo ni un verbo nuevo — es el mismo `dispatch` de los once, con un origen más (D17: el kernel contesta por donde le llegó el pedido). Al entrar recibe **su propia dirección** en el primer registro de argumento, y nada más: con eso alcanza, porque todo lo demás lo pide por la puerta. *(Ojo: hasta que D29 lo bajó a `supervised` recibía en el segundo registro un puntero a una función del kernel, y este párrafo decía eso. Sin privilegio no puede llamar a una función del kernel ni aunque supiera dónde está, así que ese camino ya no existe.)* Se comprueba mirando los reclamos después del arranque: tiene que estar el que pidió el blob y que nunca pasó por el cable — y con el blob cancelado, no.

**Y desde que existe `blob/`, el blob se compila.** Es un crate `no_std` que sale a binario plano para las dos arquitecturas (`./scripts/build-blob.sh`), con la entrada en el byte cero y sin ningún envoltorio, que es lo único que el kernel sabe cargar. Eso es lo que faltaba para que ahí adentro pueda entrar un driver: el blob de bytes escritos a mano alcanzaba para probar el mecanismo y no para nada más. **La restricción que impone es que no puede haber GOT** — la tabla de direcciones que normalmente rellena un enlazador dinámico, que acá no existe: queda con las direcciones de tiempo de enlace y el blob salta a cualquier lado. Aparece cuando el código llama a un símbolo global, que en la práctica quiere decir `memset`/`memcpy`, los que el compilador inventa solo al ver una copia o una inicialización grande. Lo hace cumplir el enlazador (`ASSERT` en `blob/blob.ld`), así que es un error de compilación y no una máquina que no arranca.
...
| **`listen`** | Andando en las dos. El kernel escucha por el cable y por el buzón, y contesta por donde le llegó (D17). El acuerdo lo publica `describe`. **Y `release` lo devuelve:** soltar la memoria del buzón hace que el kernel deje de escucharlo. Faltaba, y lo destapó el transporte de D5 —el primero que hace `listen` y después `release` en la misma corrida—: sin eso el kernel seguía leyendo y escribiendo memoria que el próximo `mem.claim` le daba a otro. |
