...
**Lo que está hecho es el mecanismo, no el contenido.** No hay ningún driver de red ni de NVMe, ni un `blob.bin` en el repo: el único blob que existe es el de prueba que genera `client.py`, que le pide memoria al kernel para demostrar que la ventanilla anda. Los drivers que nombran D19 y D20 son **lo que va a ir ahí**, no algo que ya esté — y conviene leerlos así, porque su redacción no lo aclara. La consecuencia práctica es que hoy el único transporte es el cordón umbilical: D5 dice que el agente escribe el transporte rápido, y todavía no lo escribió nadie. En QEMU no se nota porque el serie no simula baudios; en una máquina real, a 115200 baudios son unos 11 KB/s, que es exactamente el problema que D20 quiere evitar. |
