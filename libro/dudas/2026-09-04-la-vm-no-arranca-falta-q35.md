...
La práctica estaba mal escrita: copié el `-device intel-iommu` de `scripts/run-x86_64.sh` **sin copiar el `-machine q35` que está tres líneas más arriba en ese mismo script** (`scripts/run-x86_64.sh:78#-machine q35`). El script del kernel siempre lo tuvo.
