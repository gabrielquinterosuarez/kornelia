...
No es una promesa de diseño. Hay un driver de NVMe **escrito con los once verbos y nada más**, en `scripts/client.py:1732#class Nvme`, que arranca un controlador de verdad, le crea colas, lee el bloque 0, trae el resto por DMA directo a un reclamo del agente y salta ahí con `exec`. El portón lo corre en las dos arquitecturas contra un `-device nvme`.

Busca el aparato por su **clase** —`scripts/client.py:1614#NVME_CLASS = (0x01, 0x08, 0x02)`— y no por fabricante y modelo, que es P4 aplicado al bus: el aparato dice *qué hace*, y por eso el mismo driver anda contra cualquier NVMe.
