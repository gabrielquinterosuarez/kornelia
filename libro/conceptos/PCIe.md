...
Buscar por clase y no por fabricante es lo que hace que un driver ande contra hardware que no existía cuando se escribió. En Kornelia el driver de [[NVMe]] busca los tres bytes `01.08.02` —"almacenamiento / no volátil / NVMe"— y nada más: `scripts/client.py:1613#NVME_CLASS = (0x01, 0x08, 0x02)`.
...
**El kernel publica dónde se pregunta, no la respuesta.** No enumera, no arma una lista de aparatos, no tiene tabla de drivers. Da la ventana ECAM y se corre: el recorrido lo hace el agente con `mem.claim` sobre la ventana y `mem.read` adentro (`scripts/client.py:1616#def pcie_scan`). Esa función es lo más parecido a `lspci` que hay acá, y está del lado del cliente, no del kernel (D4).
