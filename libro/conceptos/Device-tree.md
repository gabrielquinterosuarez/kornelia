...
El portón bootea aarch64 **una segunda vez, sin ACPI** (`scripts/run-aarch64.sh:42#acpi=off`): entonces el firmware pasa un device tree en su lugar. Y no se conforma con que arranque — le exige llegar hasta el IOMMU contra un [[Aparato|aparato]] que hace [[DMA]] de verdad (`scripts/check.sh:453#--no-acpi`).
