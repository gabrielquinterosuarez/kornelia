...
**2. Una sección que no conoce es un error, no un silencio:** `kernel-core/src/protocol.rs:365#unknown section in what`. El comentario de al lado dice por qué: *contestar solo con lo que se reconoció, callado, sería mentir por omisión*. Si pedís `iomu` en vez de `iommu` y el kernel te contesta alegremente con las otras doce secciones, tu programa concluye que esta máquina no tiene [[IOMMU]]. Ver [[18-Lo-que-la-maquina-no-dice]].
