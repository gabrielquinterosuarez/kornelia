...
| **Dónde vive** | `kernel-core/src/fault.rs:24#pub enum Cause`, `kernel-core/src/fault.rs:73#pub struct Fault`, `kernel-core/src/protocol.rs:1716#fn write_outcome` |
...
Así que un `exec` que falla **contesta**, con `ok: true` — el pedido se atendió; que el código haya fallado es su resultado, no un fallo del pedido (`kernel-core/src/protocol.rs:1677#es su resultado, y va adentro`). La respuesta trae:
