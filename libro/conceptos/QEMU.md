...
|---|---|
| `scripts/run-x86_64.sh:105#-cpu max` | El default `qemu64` **no tiene páginas de 1 GiB**, que es lo que D12 usa para el identity map. El default de QEMU es más austero que el hardware real, no al revés. |
| `scripts/run-x86_64.sh:112#-device intel-iommu` y `scripts/run-aarch64.sh:40#MACHINE=virt,iommu=smmuv3` | D8: el IOMMU va encendido. Sin él, `dma.allow` no se puede probar contra nada. Los dos hacen lo mismo y no se parecen: ver [[IOMMU]]. |
| `scripts/run-x86_64.sh:118#-device edu,dma_mask=0xffffffffffff` | `edu` es un **motor de DMA que se maneja con cuatro escrituras**: es el aparato que hace el DMA que el IOMMU tiene que bloquear. El `dma_mask` no es opcional — ver abajo. |
| `scripts/run-x86_64.sh:123#-device nvme,serial=kornelia,drive=payload` | Un disco NVMe de verdad: es de donde D19 dice que el cargador se trae el resto, y es el aparato contra el que se prueba [[PCIe]] entero, BARs incluidos. Ver [[NVMe]]. |
| `scripts/run-aarch64.sh:115#-cpu cortex-a57 -m 512` | Un modelo concreto de ARM, con RAM explícita: en `virt` el default no alcanza. |
...
`scripts/run-x86_64.sh:101#SERIAL=(-serial stdio)` — **sin** el prefijo `mon:`, y no es un olvido.
...
- [[00-Como-mirar-una-maquina]] · [[IOMMU]] · [[Indice-de-sintomas]]
