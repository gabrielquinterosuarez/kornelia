...
| `scripts/run-x86_64.sh:86#-device intel-iommu` y `scripts/run-aarch64.sh:40#MACHINE=virt,iommu=smmuv3` | D8: el IOMMU va encendido. Sin él, `dma.allow` no se puede probar contra nada. Los dos hacen lo mismo y no se parecen: ver [[47-IOMMU-VT-d-y-SMMUv3]]. |
