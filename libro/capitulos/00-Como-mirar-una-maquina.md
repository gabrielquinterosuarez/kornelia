...
Las trece secciones que acepta `describe` son `memory`, `tables`, `claims`, `cpus`, `interrupts`, `pcie`, `cores`, `channel`, `handlers`, `exec`, `iommu`, `clock` y `cable` (`kernel-core/src/protocol.rs:346#Some("memory") => q.memory = true`). Y si le pedís una que no conoce, **contesta un error en vez de ignorarla**: contestar solo con lo que reconoció sería mentir por omisión.
